...
use graphbench::{algorithms::LinearGraphAlgorithms, degengraph::DegenGraph, graph::*};
...
impl SReachTraceOracle {
    ...
    pub fn compute_traces(&self, inner: &Vec<Vertex>, graph:&DegenGraph) -> FxHashMap<Vec<Vertex>, usize>  {
        ...
        // println!("Computing traces via sreach data");
...
mod  tests {
    ...
    fn shattered_test_DNC () {
        let mut graph = EditGraph::from_gzipped("networks/DNC-emails.txt.gz").expect("File not found.");
        graph.remove_loops();
    ...
    fn traces_test_Yeast () {
        let mut graph = EditGraph::from_gzipped("networks/Yeast.txt.gz").expect("File not found.");
        graph.remove_loops();
